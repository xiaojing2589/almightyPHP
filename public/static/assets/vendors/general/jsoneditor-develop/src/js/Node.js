...
export class Node {
  ...
  getName () {
    return !this.parent
      ...
      : (this.parent.type !== 'common.builder.common.form.item.array')
        ? this.field
        : this.index
  }
  ...
  findNode (jsonPath) {
    ...
    while (node && path.length > 0) {
      ...
      if (typeof prop === 'number') {
        if (node.type !== 'common.builder.common.form.item.array') {
  ...
  setValue (value, type) {
    ...
    if (this.type === 'common.builder.common.form.item.array') {
      // array
      if (!this.childs) {
        this.childs = []
      }
  ...
  setInternalValue (internalValue) {
    ...
    if (internalValue.type === 'common.builder.common.form.item.array') {
      // array
      if (!this.childs) {
        this.childs = []
      }
  ...
  expandPathToNode () {
    ...
    while (node && node.parent) {
      ...
      const index = node.parent.type === 'common.builder.common.form.item.array'
        ? node.index
  ...
  changeType (newType) {
    ...
    if ((newType === 'string' || newType === 'auto') &&
      ...
      } else if (newType === 'common.builder.common.form.item.array') {
        if (!this.childs) {
          this.childs = []
        }
  ...
  _getDomValue () {
    ...
    if (this.dom.value && this.type !== 'common.builder.common.form.item.array' && this.type !== 'object') {
  ...
  _updateDomValue () {
    ...
    if (domValue) {
      ...
      const isEmpty = (String(this.value) === '' && this.type !== 'common.builder.common.form.item.array' && this.type !== 'object')
      if (isEmpty) {
      ...
      // update title
      if (valueType === 'common.builder.common.form.item.array' || valueType === 'object') {
  ...
  _updateDomField () {
    ...
    if (domField) {
      ...
      const isEmpty = (String(this.field) === '' && this.parent.type !== 'common.builder.common.form.item.array')
      if (isEmpty) {
  ...
  _createDomTree () {
    ...
    if (this.type !== 'object' && this.type !== 'common.builder.common.form.item.array') {
   ...
   */
  _hasChilds () {
    return this.type === 'common.builder.common.form.item.array' || this.type === 'object'
  }
    ...
    if (this.editable.value) {
      items.push({
        ...
        submenu: [
          ...
          {
            text: translate('common.builder.common.form.item.array'),
            className: 'jsoneditor-type-array' +
                (this.type === 'common.builder.common.form.item.array' ? ' jsoneditor-selected' : ''),
            title: titles.array,
            click: function () {
              node._onChangeType('common.builder.common.form.item.array')
            }
          },
          {
  ...
  _getType (value) {
    if (value instanceof Array) {
      return 'common.builder.common.form.item.array'
    }
