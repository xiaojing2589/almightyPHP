    ...
    */
    ...
    var handleLinkage = function () {
        $('.js-linkage').each(function () {
            ...
            // 所有的下拉框对象
            var _select = _this.find('.js-linkage-select');

            // 顶级父节点值
            var topParentValue = (_this.data('topParentValue') != undefined || _this.data('topParentValue') != null) ? _this.data('topParentValue') : 0;

            // 数据的id 字段名
            var xid = _this.data('id') ? _this.data('id') : 'id';

            // 数据的显示文本字段名
            var xtext = _this.data('text') ? _this.data('text') : 'text';

            // 数据的父节点字段名
            var xpid = _this.data('pid') ? _this.data('pid') : 'pid';

            // 下拉框所有数据
            var myData = _this.data('datasourcename');
            myData = window[myData];

            // 默认选中
            var defaultname = _this.data('defaultname');
             ...
             * 根据id 查找所有父节点数据
             * @param id
             * @returns {Array}  指定ID的父级数据
             */
            function getParent(id) {
                var parents = [];
                for (var i = 0, len = myData.length; i < len; i++) {
                    if (id == myData[i][xid]) {
                        if (myData[i][xpid] == topParentValue) {
                            parents.push(myData[i]);
                            break;
                        } else {
                            parents = getParent(myData[i][parentIdKey]);
                            parents.push(myData[i]);
                        }
                    }
                }
            ...
            /**
             *  当select选定一个option后，根据该option的值，查找 parentID 为该 optionID 的项，生成option，插入下级select。
             * @param id 父级id
             * @param index 父级select2索引
             * @param selectID 默认选中项
             */
            function initChild(id, index,selectID) {

                var pID = null == id ? topParentValue : id;

                // 获取级别数据
                var ds = getChilds(pID);

                // 清空
                clearSelect(index);

                // 获取该级别对象
                var child = select2Arr[index];

                if(child){
                    for (var i = 0, len = ds.length; i < len; i++) {
                        var currentObj = ds[i];
                        if(selectID == undefined || selectID == null){
                            var newOption = new Option(currentObj[xtext], currentObj[xid]);
                        }else{
                            if(selectID == currentObj[xid]){
                                var newOption = new Option(currentObj[xtext], currentObj[xid],true,true);
                            }else{
                                var newOption = new Option(currentObj[xtext], currentObj[xid]);
                            }
                        }
            ...
            function clearSelect (index) {
                for (var i = index, len = select2Arr.length; i < len; i++) {
                    select2Arr[i][0].length = 1;
                    // select2Arr[i][0].length = 0;
                    // select2Arr[i].val(null).trigger('change');
                }
            }
             ...
             *         id:         选中项ID
             */
            function initSelect(dataId) {
                $.each(dataId,function (i, v) {
                    if (i == 0) {
                        initChild(null, 0,v);
                    } else {
                        initChild(dataId[i - 1], i,v);
                    }
                });
            }

            // 初始化select2
            _select.each(function (i, v) {
                ...
                _select2.on('select2:select', function (e) {
                    ...
                    // 初始化下一个 select2 数据
                    initChild(datas['id'],i+1);
                });

                select2Arr[i] = _select2;
            });
